...
sub preHeader {
 ...
 my $sref=$Tasks{$CurTaskID}->{preHeader}||=[sub{
  ...
  return call('L2',$this->{getline}->($fh,$l)); L2:
 }];
 &{$sref->[0]};
 return $sref->[1];
}
...
sub getHeaderExec {
 ...
 my $sref=$Tasks{$CurTaskID}->{getHeaderExec}||=[sub{
  ...
  checkHeader($fh);
  if ($l=~/^\.(?:\015\012)?$/) {   
   return call('L2',getBodyDone($fh,1)); L2:
  } else {
...
sub reply {
 ...
 my $sref=$Tasks{$CurTaskID}->{reply}||=[sub{
  ...
  if ($DetectInvalidRecipient && $l=~/$DetectInvalidRecipient/i) {
   ...
   return call('L3',checkRBL($client)) if $RBLPosition==2; L3:
   return if checkNonLate($client,0)<0;
  } elsif ($l=~/^235/) {
...
sub resetStats {
 ...
 my @providers=('RWL',@rwllist,'RBL',@rbllist,'URIBL',@uribllist);
